...
def run_empirical_power_experiment(config: ExperimentConfig) -> EmpiricalSizeResult:
    """Run the full empirical power experiment.

    This generates data under the alternative (deviated distribution) and tests under the null.
    """

    true_params = DeviatedParams(
        mu=config.mu_true,
        sigma=float(np.sqrt(config.s2_true)),
        alpha=config.alpha_true,
        tau=config.tau_true,
    ...
    for order in config.orders:
        ...
        def _single(iteration: int) -> tuple[bool, bool]:
            ...
            observed_data = DeviatedHybridLogNormalPareto.generate_sample(
                config.n_data, true_params, seed=iter_seed
            )
            observed_psi = AuxiliaryStatistics.compute_vector(observed_data, order)
        ...
        empirical_power_1 = rejections_1 / config.M
        empirical_power_1opt = rejections_1opt / config.M
        ...
        order_results.append(
            OrderResult(
                ...
                sigma_est=float(sigma_hat),
                alpha_est=float(alpha_hat),
                empirical_size_test1=float(empirical_power_1),
                empirical_size_test1opt=float(empirical_power_1opt),
                rejections_test1=int(rejections_1),
                rejections_test1opt=int(rejections_1opt),
                M_repetitions=config.M,
                nominal_size=config.alpha,
        ...
        if config.verbose:
            print(f"Order {order} Results:")
            print(f"  Test 1: {empirical_power_1:.4f}")
            print(f"  Test 1opt: {empirical_power_1opt:.4f}")
